...
install: $(VENV) ## Install open-fang + harness_core editable
	$(PIP) install -e ./harness_core
	$(PIP) install -e '.[dev]'

test: install ## Run unit + integration tests
	$(PYTEST) -q tests

lint: install ## Lint
	$(VENV_BIN)/ruff check src tests

run: install ## Start FastAPI locally on $(PORT)
	$(UVICORN) open_fang.app:app --reload --port $(PORT)
